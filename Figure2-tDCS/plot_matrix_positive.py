...
vmax=0.16
averagesize=50
T = '/home/hanlu/Desktop/Freiburg/tDCS-and-learning-NEST2.16/tDCS-only-np/EE_matrix/'
for idx in [0,1,2,3]:
    time = times[idx]
    matrix = np.load(T+"EE_matrix_step_"+str(time)+"_seed_1.npy")*10.
    matrix_reduced = equalize(matrix)
    axes[idx].imshow(equalize(matrix,size=10)[0:150,0:150],vmin=vmin,vmax=vmax)
...
img = ax4.imshow(equalize(matrix,size=10)[0:150,0:150],vmin=vmin,vmax=vmax)
...
plt.savefig("/home/hanlu/Desktop/Freiburg/tDCS-and-learning-NEST2.16/Figures/Figure2-tDCS/matrix-positive-reduced.svg")
plt.show()

...
for ii in [0,1,2]:
    rate = np.load(T1+"1000rates2_seed_"+str(ii)+".npy")
    conn = np.load(T1+"1000g2g2_seed_"+str(ii)+".npy")*10.
...
for ii in [0,1,2]:
    rate = np.load(T2+"1000rates2_seed_"+str(ii)+".npy")
    conn = np.load(T2+"1000g2g2_seed_"+str(ii)+".npy")*10.
...
for ii in [3]:
    rate = np.load(T2+"1000rates2_seed_"+str(ii)+".npy")
    conn = np.load(T2+"1000g2g2_seed_"+str(ii)+".npy")*10.
    ax2.plot(t,rate,color=yellow,label=str(elec_field_strens[ii])+"mV")
    ax5.plot(t,conn,color=yellow,label=str(elec_field_strens[ii])+"mV")    
...
for ii in [0,1,2]:
    rate = np.load(T3+"1000rates2_seed_"+str(ii)+".npy")
    conn = np.load(T3+"1000g2g2_seed_"+str(ii)+".npy")*10.
...
ax4.set_ylabel('$\Gamma_\mathrm{engram+sti}$')
ax4.set_yticks(np.arange(0.04,0.20,0.05))
ax5.set_xlabel("Time (s)")
ax5.set_xticks(np.arange(700,1600,200))


plt.savefig("/home/hanlu/Desktop/Freiburg/tDCS-and-learning-NEST2.16/Figures/Figure6-g1g2g3/g2g2curves.svg")
plt.show()
